...
use swc_core::{
    ...
    ecma::{
        ast::{Module, ModuleDecl, ModuleItem, ImportDecl, Expr, Lit, Callee, CallExpr, Str, ExprOrSpread},
...
static DEFAULT_EXTENSIONS: Lazy<Vec<&'static str>> = Lazy::new(|| {
    vec!["js", "ts", "jsx", "tsx", "json"]
});
...
fn process_transform(node_span: Span, cwd: PathBuf, filename: FileName, required_file_path: String) -> TransformResult {
    ...
    let mut required_file_full_path= resolve_required_file_path(cwd.clone(), filename.to_string(), required_file_path.clone());

    // 是否是默认扩展名称, 否则默认兜底为 .js 后缀
    if !is_default_extension(&required_file_full_path) {
        required_file_full_path = PathBuf::from(format!("{}.js", required_file_full_path.to_str().unwrap().to_string()));
    }
            ...
            if let Expr::Ident(i) = &**e {
                if i.sym == *"require" && n.args.len() == 1 {
                    if let Expr::Lit(Lit::Str(module_name)) = &*n.args[0].expr {
                        ...
                        let result = process_transform(n.span, self.cwd.clone(), self.filename.clone(), required_file_path);
...
fn replace_to_js_extension(path: &PathBuf) -> PathBuf {
    ...
    if required_file_ext == Some("ts") || required_file_ext == Some("jsx") || required_file_ext == Some("tsx") {
        p.set_extension("js");
    }
    return p;
}
