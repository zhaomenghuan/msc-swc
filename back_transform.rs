...
mod tests {
    ...
    fn test_transform_sync() {
        ...
        const aa = require("xxx")
            function add(a, b,require) {
            require("xxx")
                return a + b;
            }
        "#;

        let string = r#"{"cwd":"/Users/zuosh/code/zuosh/msc_test","filename":"app.js","sourceMaps":true,"isModule":true,"swcrc":false,"jsc":{"parser":{"syntax":"ecmascript","jsx":false},"transform":{}},"env":{"targets":["safari >= 9","android >= 4.1"]},"module":{"type":"commonjs","strictMode":true}}"#;
        let opts: Vec<u8> = Vec::from(string.as_bytes());

        let cus_str = r#"{"externalPackages":["react","@mtfe/msc-rlist"]}"#;
        let custom_opts: Vec<u8> = Vec::from(cus_str.as_bytes());


        let result = transform_sync(s.to_string(), opts, Some(custom_opts)).unwrap();

        assert_eq!(result.code, "function add(a, b) {\n  return a + b;\n}\n");
        assert_eq!(result.map, None);
        assert_eq!(result.metadata.requires, vec!["require", "external"]);
    }
}
